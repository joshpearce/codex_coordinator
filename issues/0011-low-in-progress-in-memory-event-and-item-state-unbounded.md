...
Originally, `EventLog.events` and `ApprovalBroker.items` grew for the lifetime of
the service. High-volume token, diff, or item notifications could exhaust memory
and make event polling increasingly expensive.
...
memory currently means discarding history rather than moving it.
